...
static void CreateMesh(const aiScene *scene, const aiNode *node, const pxr::SdfPath &path, pxr::default_stage stage)
{
  ...
  for (int i = 0; i < node->mNumMeshes; i++)
  {
    ...
    {
      dt::log::debug("Sanitized mesh name from '{}' to '{}'", mesh->mName.C_Str(), meshName);
    }
    else
    {
      dt::log::debug("Got mesh {}", meshName);
    }
    ...
    dt::log::debug("Defined USD mesh at {}", usdMesh.GetPath().GetString());
  }
...
void dt::scene::Robot::FromURDF(pxr::default_stage stage, const std::string &path)
{
  urdf::Model model;
  ...
  for (const auto &[name, link] : model.links_)
  {
    ...
    this->xforms[name] = xform.AddTransformOp();
  ...
  log::event("Converted URDF to USD with {} links", this->xforms.size());
}